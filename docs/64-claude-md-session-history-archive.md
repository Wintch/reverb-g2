# CLAUDE.md session-history archive (superseded headers, moved out 2026-08-20)

This is the full chain of `START HERE`/`PREVIOUS`/`SUPERSEDED`/`ADDENDUM`/
`MILESTONE`/`RESOLVED`/`STATUS` session-open blocks that used to stack up at the
top of `CLAUDE.md`, oldest at the bottom, newest at the top — moved here verbatim
on 2026-08-20 because the file crossed the 150k-char context limit. `CLAUDE.md`
now keeps only the single current header plus the evergreen project sections;
this file is the full prior narrative, unedited, for when older context is needed.
Read `docs/pruebas.jsonl` for the underlying session log this all summarizes.
