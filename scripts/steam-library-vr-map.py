...
def try_web_api() -> tuple[list[dict], str] | None:
    ...
    url = (
        ...
        "&include_played_free_games=1&include_free_sub=1&skip_unvetted_apps=false"
    )
    try:
        req = urllib.request.Request(url, headers={"User-Agent": USER_AGENT})
        with urllib.request.urlopen(req, timeout=20) as resp:
...
def make_record(appid, owned_info, installed_name, docs23_entry, api_entry, fetch_status):
    ...
    return {
        ...
        "owned_sources": (owned_info or {}).get("owned_sources", []),
...
def write_outputs(records: list[dict], stats: dict, owned_route: str, owned_count: int):
    ...
    lines.append(
        ...
        "appids trigger network requests. For the full owned list export "
        "`STEAM_API_KEY` first (e.g. `set -a; . ~/.config/reverb-g2/steam.env; set +a` "
        "-- the key lives OUTSIDE the repo, never commit it); without it the map "
        "falls back to localconfig.vdf only. Does not touch Steam, Monado, or "
...
def main():
    ...
    # Owned list = UNION of every source that answers, with per-app provenance.
    # Measured 2026-08-21: no single source is complete. GetOwnedGames (even with
    # skip_unvetted_apps=false) omits free titles never played, demos, mods and
    # videos that localconfig.vdf lists; localconfig misses old licenses never
    # touched on this install that the API has (86 of them). The web routes are
    # preferred for names/playtime; localconfig fills the rest.
    sources: dict[str, list[dict]] = {}
    xml_result = try_profile_xml()
    if xml_result:
        sources["profile-xml"] = xml_result[0]
    api_result = try_web_api()
    if api_result:
        sources["web-api"] = api_result[0]
    local_ids = parse_localconfig_appids(LOCALCONFIG_PATH)
    if local_ids:
        sources["localconfig"] = [{"appid": i, "name": "", "hours_on_record": None} for i in local_ids]

    merged: dict[int, dict] = {}
    for src in ("web-api", "profile-xml", "localconfig"):
        for g in sources.get(src, []):
            e = merged.setdefault(int(g["appid"]), {"appid": int(g["appid"]), "name": "",
                                                    "hours_on_record": None, "owned_sources": []})
            e["owned_sources"].append(src)
            if not e["name"] and g.get("name"):
                e["name"] = g["name"]
            if e["hours_on_record"] is None and g.get("hours_on_record") is not None:
                e["hours_on_record"] = g["hours_on_record"]
    owned = list(merged.values())
    per_source = ", ".join(f"{k}={len(v)}" for k, v in sources.items()) or "none"
    missing = []
    if "web-api" not in sources:
        missing.append("web-api (no STEAM_API_KEY in the environment, or the call failed)")
    if "profile-xml" not in sources:
        missing.append("profile-xml (game-details privacy not public)")
    owned_route = f"UNION of {per_source}" + (f"; unavailable: {'; '.join(missing)}" if missing else "")
