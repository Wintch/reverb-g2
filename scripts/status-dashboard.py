...
DEMO_LAUNCHES = [
    ...
    ("Dreams of Dali", "591360", "6dof", "approved", "Headset-only gaze-dwell, no controllers. 2026-08-29 15:15 worn on THIS button (scale 100, anchor profile): 89-90 fps in 6/8 20-s windows, dips to 79/85 when the GPU hits the 250 W cap (91-96 %) -- keep the cap at 250 W for the booth; pacer 0.02 % late; SLAM 30 Hz, 0 guard trips, max 1.57 m. Wearer: solido. RULES: lit room (light-preflight.sh); headset still on the desk until the title has loaded (~60 s); then the guest puts it on, sits looking straight ahead and the operator presses the Recentrar button (xrizer 0008, worn-validated 2026-08-29: 90-degree test, 0.2 s). 2026-08-30: recentres by itself ~2 s after donning (xrizer 0009, validated worn 2026-08-29 22:57); 🎯 stays as fallback. 2026-09-07 PROFILE CHANGE (both worn-approved the same day, after the 3060 Ti swap put the booth on a 210 W cap): render scale 100 -> 85, and Basalt detection density 30/3 -> 30/2. Scale 85 restores 7/8 20-s windows at >= 89 fps (scale 100 had fallen to 2/8 under the new cap); wearer: se juega muy bien, la resolucion no molesta tanto. Density 30/2 fixes the felt redraw -- it was pose staleness, age_out p50 100.8 -> 36.7 ms; wearer: medium movement ahi se ve perfecto. Trade accepted by the wearer: half the landmarks, so an anchor correction now reads 1.53 m instead of 1.15 m. STILL TO CONFIRM on the next worn run: 30/2 was only ever worn at 4 Basalt threads and this profile runs 6."),
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .camera-thumb { position:relative; aspect-ratio:4/3; background:var(--bg); border-radius:4px; overflow:hidden; }
