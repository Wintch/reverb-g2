...
Sixteen patches on top of Monado `main` @ `735e29e4e` (the SHA `bootstrap-lab.sh sources`
...
on top of it, referenced by their own commit messages as "0017" work. None of that is
exported as a loose `.patch` file yet. Don't assume this directory's numbering reflects the
full state of the branch; check `git log` on the lab machine before assuming something past
0015 doesn't exist yet.
...
| 0016 | (unfiled, verified live 2026-08-11, everyday system) | Corrects 0015's two wrong guesses (slot offset `+tcam_count` → flat `+2`; exposure/gain `400/1` → `6000/100`) and adds a clock-domain fix (`+cam_hw2mono`) for constellation frame timestamps. Confirmed live: `Controller Blob Cam N` panels went from solid black to real tracked LED blobs, and RANSAC-PnP started recovering real device poses (2000+ consecutive samples, reprojection error 0.04–0.10px). The clock fix is only partially closed — see the patch's own commit message and `docs/pruebas.jsonl` T149 for a second, unexplained device-specific timestamp bug found in the same session that still blocks `get_tracked_pose` from reporting `position_tracked=true`. |

All sixteen apply with plain `git am` onto the pinned SHA and build with zero warnings.
