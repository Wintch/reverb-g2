...
## 0021 — stereo audio playback (2026-08-26)

| patch | what |
|---|---|
| `0021-360-viewer-stereo-audio-playback-pipewire-pa-simple.patch` | `HELLO_XR_AUDIO=1` — plays the file's stereo audio track through PipeWire's `pa_simple` shim, synced to video. Full detail (design, sync/loop/seek mechanics, what's verified vs. not) in `docs/02-player-360.md`'s "Stereo audio" section. Off by default with zero behavior change. Build-verified (`ninja hello_xr`, clean under `-Wall` + this project's `-Werror=` set, links `libswresample`/`libpulse-simple`) and `pa_simple` confirmed to reach this box's live PipeWire-Pulse sink via a standalone test — the decode+resample+sync path itself has **not** been run end to end (needs a real XR session) and audibility/A-V-sync/pause/quit behavior is **not yet verified with a human in the headset**. Commit `1ea7f1291`. |
